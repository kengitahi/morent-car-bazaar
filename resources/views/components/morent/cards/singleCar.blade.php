<div class="p-4 mb-8 rounded-lg shadow-2xl sm:mb-4">
    <header class="flex items-center justify-between gap-4 mb-2 flex-cols">
        <div class="space-y-1">
            <h2 class="text-lg font-bold text-secondary-500">
                <a href="{{ route('car.show', $car['id']) }}">
                    {{ $car['name'] }}
                </a>
            </h2>
            <p class="text-sm capitalize text-secondary-400">
    ...
    <main class="space-y-4">
        <div>
            <a href="{{ route('car.show', $car['id']) }}">
                <img alt="{{ $car['name'] }}" class="rounded-lg" src="{{ asset($car['images'][0]) }}">
            </a>
        </div>
        <div class="grid grid-cols-2 gap-1 lg:gap-2">
            <div class="flex items-center gap-1">
                ...
                <p class="text-sm capitalize text-secondary-400">{{ $car['fuel'] }}</p>
            </div>
            <div class="flex items-center gap-1">
                <div class="text-secondary-400">
                ...
                <p class="text-sm text-secondary-400">
                    {{ $car['fuel'] == 'gasoline' ? $car['fuel_capacity'] . ' L' : $car['range'] . ' Kms' }}
                </p>
                <!-- <p class="text-sm text-secondary-400">{{ $car['fuel_capacity'] }} {{ $car['fuel'] == 'gasoline' ? 'L' : 'Kms' }}</p> -->
            </div>
            <div class="flex items-center gap-1">
                <div class="text-secondary-400">
                    <svg fill="none" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <path d="M12 6c-3.312 0-6 2.688-6 6s2.688 6 6 6 6-2.688 6-6-2.682-6-6-6Z" fill="currentColor" />
                        <rect fill="#fff" height="8" rx="4" width="8" x="8" y="8" />
                        <path d="M11 17h2v4h-2zm6-6h4v2h-4zM3 11h4v2H3z" fill="currentColor" />
                    </svg>
                </div>
                <p class="text-sm capitalize text-secondary-400">{{ $car['steering'] }}</p>
            </div>
            <div class="flex items-center gap-1">
                <div class="text-secondary-400">
                    <svg fill="none" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                ...
                <p class="text-sm text-secondary-400">{{ $car['capacity'] }} people</p>
            </div>
        </div>
        <div class="flex justify-between">
            <div class="flex flex-col gap-0.5">
                @isset($car['discounted_price'])
                    <p>
                        <span class="text-base font-bold text-secondary-500">${{ $car['discounted_price'] }}/</span><span
                            class="text-sm text-secondary-400">day</span>
                    </p>
                    <p class="text-sm line-through text-secondary-400">${{ $car['regular_price'] }}</p>
                @else
                    <p>
                        <span class="text-base font-bold text-secondary-500">${{ $car['discounted_price'] }}/</span><span
                            class="text-sm text-secondary-400">day</span>
